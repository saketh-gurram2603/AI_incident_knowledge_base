...
def _calculate_confidence(results: list[dict]) -> float:
    """
    Weighted average of top-5 similarity scores.
    Position 1 carries 5× weight, position 5 carries 1× weight.
    This rewards a strong top-1 hit significantly over a mediocre spread.
    """
    if not results:
        return 0.0
    top = results[:5]
    scores = [r.get("similarity_score", 0.0) for r in top]
